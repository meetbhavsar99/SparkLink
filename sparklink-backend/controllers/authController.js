...
exports.login = async (req, res) => {
    try {
        ...
        if (!user) {
            await logsController.createLog(null, "Login Failed", `Invalid email: ${email}`, "error");
            return res.status(401).json({ message: "Invalid credentials" });
        }

        const isMatch = await bcrypt.compare(password, user.password);
        if (!isMatch) {
            await logsController.createLog(user.user_id, "Login Failed", `Incorrect password attempt`, "error");
            return res.status(401).json({ message: "Invalid credentials" });
        }

        // Successful login
        await logsController.createLog(user.user_id, "Login Success", "User logged in", "user");

        res.status(200).json({ message: "Login successful", user });
    } catch (error) {
        await logsController.createLog(null, "System Error", error.message, "error");
        res.status(500).json({ message: "Internal server error" });
    }
};
...
exports.createProject = async (req, res) => {
    try {
        ...
        await logsController.createLog(user_id, "Project Created", `Project ${project_name} created successfully`, "action");

        res.status(201).json({ message: "Project created successfully", project: newProject });
    } catch (error) {
        await logsController.createLog(req.body.user_id, "Project Creation Failed", error.message, "error");
