// Controller to fetch or create user profiles dynamically based on their roles
...
// Fetch profile and project details for a given user based on role
exports.getProfile = async (req, res) => {
  try {
    ...
    const user = await User.findOne({
      where: { user_id },
      include: [{ model: Role, as: "roleDetails", attributes: ["role_desc"] }],
    });
    ...
    if (!role) {
      ...
      return res
        .status(404)
        .json({ message: `Role ID ${user.role} not found in t_rolesmst` });
    }
...
// Manually create a profile for a user (used only if not already created)
