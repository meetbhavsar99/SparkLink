// Controller for handling project milestone lifecycle: create, update, complete, resume, and delete.
// Also handles role-based milestone access, milestone progress computation, and project filtering.
...
exports.createMilestone = async (req, res) => {
  // Validate each milestone entry before creation
  // Ensure milestone counter exists or create it before assigning unique milestone ID
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: "Milestone(s) required and cannot be empty" });
    }

    const milestoneData = [];

    for (const milestone of milestoneList) {
      const { proj_id, milestone_desc, milestone_title, end_date } = milestone;

      const isValidMilestoneTitle =
        !ValidationUtil.isEmptyString(milestone_title) &&
        !ValidationUtil.isConsecSplChar(milestone_title) &&
        ValidationUtil.isValidString(milestone_title, 5, 250);

      const isValidMilestoneDesc =
        !ValidationUtil.isEmptyString(milestone_desc) &&
      ...
      if (!isValidDate) {
        ...
        return res.status(400).json({
          message:
            "Please enter a valid Milestone Title - You may not enter consecutive special characters",
        });
      } else if (!isValidMilestoneDesc) {
        return res.status(400).json({
          message:
            "Please enter valid Milestone Description - You may not enter consecutive special characters",
        });
      }
    ...
    res.status(201).json({
      message: "Project Milestone(s) created successfully",
      milestoneData,
    });
  } catch (error) {
    ...
    console.error(error);
    res.status(500).json({
      message: "Error creating project milestone",
      error: error.message,
    });
  }
};

// Fetch all active milestones for a given project, ordered by end date
exports.FetchMilestone = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Milestone(s) fetched successfully", milestoneData });
  } catch (error) {
    console.error(error);
    res.status(500).json({
      message: "Error fetching project milestone(s)",
      error: error.message,
    });
  }
};

// Validate and update milestone details (title, desc, end_date) based on project and milestone ID
exports.UpdateMilestone = async (req, res) => {
  try {
    const { milestoneList } = req.body;

    const isValidMilestoneTitle =
      !ValidationUtil.isEmptyString(milestoneList.milestone_title) &&
    ...
    const isValidMilestoneDesc =
      !ValidationUtil.isEmptyString(milestoneList.milestone_desc) &&
    ...
    const updatedData = await Milestone.update(
      ...
      {
        where: {
          proj_id: milestoneList.proj_id,
          milestone_id: milestoneList.milestone_id,
        },
      }
    );
    res
      .status(200)
      .json({ message: "Milestone updated successfully", updatedData });
  } catch (error) {
    console.error(error);
    res
      .status(500)
...
// Soft delete milestone by marking it inactive
exports.DeleteMilestone = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Milestone deleted succesfully", deleteData });
  } catch (error) {
    console.error(error);
    res
      .status(500)
...
// Mark a milestone as completed if it's active and not already completed
exports.CompleteMilestone = async (req, res) => {
  try {
    ...
    if (ms.is_active === "Y" && ms.is_completed === "N") {
      ...
      res.status(200).json({
        success: true,
        message: "Milestone marked as complete succesfully",
        completeData,
      });
    } else {
      res.status(200).json({
...
// Fetch all milestones and corresponding project data for a specific project
exports.ProjMilestones = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      message: "Project Milestone(s) fetched successfully",
      projMilestoneData,
      projData,
    });
  } catch (error) {
...
// Determine if the current user has permission to manage milestones for the given project
  ...
  try {
    ...
    if (role === 3) {
      ...
      if (supervisor_exists === 1) {
        return res
          .status(200)
          .json({ success: true, message: "Valid User", access_val: "SU" });
      } else {
        return res
          .status(200)
      ...
      if (student === 1) {
        return res
          .status(200)
          .json({ success: true, message: "Valid User", access_val: "ST" });
      } else {
        return res
          .status(200)
          .json({ success: false, message: "Invalid User", access_val: "I" });
    ...
    return res.status(500).json({
      message: "Error fetching user role access",
      error: error.message,
    });
  }
};

// Calculate project milestone completion percentage based on active vs completed milestones
exports.ProjectProgress = async (req, res) => {
  try {
    ...
    const progress = Math.round(
      (completedMilestoneCount / activeMilestoneCount) * 100
    );
    ...
    return res.status(200).json({
      message: "Project Milestone Progress fetched successfully",
      projProgressData,
    });
  } catch (error) {
    return res.status(500).json({
...
// Reopen a previously completed milestone by marking it as not completed
exports.ResumeMilestone = async (req, res) => {
  try {
    ...
    if (ms.is_active === "Y" && ms.is_completed === "Y") {
      ...
      res.status(200).json({
        success: true,
        message: "Milestone resumed succesfully",
        completeData,
      });
    } else {
      res.status(200).json({
...
// Filter projects based on partial project name match for the authenticated user
