...
const ViewUserComponent = () => {
  ...
  const filterByRole = (roleName) => {
    setRole(roleName);
    const filtered = roleName === "All"
      ? allUsers
      : allUsers.filter((user) => user.roleName === roleName);
  ...
  const handleDeleteClick = async (user) => {
    try {
      ...
      if (response.status === 200) {
        ...
        setAllUsers(allUsers.filter((u) => u.user_id !== user.user_id));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const updatedUser = {
        ...
        is_active: selectedUser.is_active ? "Y" : "N",
      };
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await axios.post("/api/users/bulk-delete", { user_ids: selectedUsers });
      Swal.fire("Success", "Users deleted successfully", "success");
      setAllUsers(allUsers.filter((u) => !selectedUsers.includes(u.user_id)));
      setFilteredUsers(filteredUsers.filter((u) => !selectedUsers.includes(u.user_id)));
      setSelectedUsers([]);
    } catch (err) {
  ...
  const handleExportCSV = () => {
    const csvContent = "data:text/csv;charset=utf-8," +
      ["User ID,Username,Name,Email,Role,Status"]
        .concat(
          allUsers.map((user) =>
            [user.user_id, user.username, user.name, user.email, user.roleName, user.is_active ? "Active" : "Inactive"].join(",")
          )
        )
        .join("\n");
  ...
  <div className="content-container">
    <div className="container-fluid user-management-wrapper">
      <h1 className="text-center user-title mb-4">User Management</h1>
